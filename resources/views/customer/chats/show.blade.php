...
@section('title', 'Chat with ' . $chat->consultant->name)
...
<style>
    ...
    /* Customer's OWN messages (sender_type === 'App\Models\Customer') */
    .message.customer {
        background-color: #dcf8c6; /* Light, pleasant green */
        align-self: flex-end; /* Aligns customer's messages to the right */
        margin-left: auto; /* Pushes customer's messages to the right */
    ...
    /* Consultant's messages (sender_type === 'App\Models\Consultant') */
    .message.consultant {
        background-color: #e0e9f1; /* Soft blueish-gray for consultant messages */
        align-self: flex-start; /* Aligns consultant messages to the left */
        margin-right: auto; /* Pushes consultant messages to the left */
...
<div class="chat-wrapper">
    <div class="chat-header">
        Chat with {{ $chat->consultant->name }}
    </div>

    <div class="chat-container" id="chat-container">
        @foreach($messages as $message)
            {{--
                Logic for message alignment and color:
                - If the message sender is a Customer (this user), align right and use green background.
                - If the message sender is a Consultant, align left and use grey background.
            --}}
            <div class="message {{ $message->sender_type === 'App\Models\Customer' ? 'customer' : 'consultant' }}">
    ...
    <div class="chat-input-area">
        ...
        <form action="{{ route('customer.chat.messages.store', $chat) }}" method="POST" enctype="multipart/form-data" id="chat-form">
            @csrf
